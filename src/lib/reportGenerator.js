...
const PANELS = [
  ...
  },
  {
    ...
    tests: [
      ...
      { name: 'Epigenetic Clock — Order from a certified epigenetic test lab (home kit)', code: 'EPIC' },
    ],
...
export function buildWhatsAppText(report) {
  ...
  lines.push(
    ...
    ``,
    `After testing, upload to AROGYOS to fill all data gaps automatically.`,
  )
  return lines.join('\n')
}
...
export function buildClipboardText(report) {
  ...
  lines.push(
    ...
    `(price varies by lab and city — compare NABL-certified labs near you)`,
    ``,
    `WHERE TO GET TESTED`,
    `• Any NABL-certified diagnostic lab in your city`,
    `• Ask for home blood collection when booking`,
    `• Government hospital labs (AIIMS, civil hospitals) are trusted and affordable`,
    `• Show the lab technician this complete panel list at the counter`,
    ``,
    `After testing: Upload your PDF report to AROGYOS.`,
    `All data gaps fill automatically once uploaded.`,
    ``,
    `Generated by AROGYOS — Biological Age Reversal`,
  )
  return lines.join('\n')
}
